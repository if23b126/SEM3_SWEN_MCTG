...
public class GameServiceImpl extends AbstractService implements GameService {
    ...
    public Response acceptTrading(Request request) {
        ...
        Response response;
        if(userRepository.checkIfUserIsLoggedIn(token)) {
            String tradingID = request.getPathname().substring(10);
            String offer = gameRepository.getCardFromTradingID(tradingID);
            ...
            } else if (!gameRepository.checkIfTradingExists(tradingID)) {
